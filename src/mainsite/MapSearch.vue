...
// current user position helper
const currentPosition = computed(() =>
  latitude.value && longitude.value ? { lat: Number(latitude.value), lng: Number(longitude.value) } : null
)
...
function loadCachedLocation() {
  try {
    ...
    if (v) {
      ...
      if (Array.isArray(arr) && arr.length === 2) lastKnown.value = [Number(arr[0]), Number(arr[1])]
    }
  } catch (e) {
...
const initializeMap = () => {
  ...
  userMarker = L.marker(placeholder, { icon: userIcon }).addTo(map.value).bindPopup(lastKnown.value ? 'Last known location' : 'Locating you...').openPopup()
...
const highlightUserCityBoundary = async (lat: number, lon: number) => {
  if (!map.value) return
  try {
    const geoRes = await fetch(`https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${lon}&apiKey=${GEOAPIFY_API_KEY}`)
    const geoData = await geoRes.json()
    const props = geoData.features?.[0]?.properties || {}
    const cityName = props.city || props.town || props.village || props.county || props.state_district || props.state
    if (!cityName) { errorMsg.value = 'Unable to detect city'; return }
    ...
    if (!osmData.features?.length) { errorMsg.value = `No boundary found for ${cityName}`; return }
    const boundaryFeature = osmData.features[0]
    if (cityBoundaryLayer.value && map.value.hasLayer(cityBoundaryLayer.value)) map.value.removeLayer(cityBoundaryLayer.value)
    cityBoundaryLayer.value = L.geoJSON(boundaryFeature, { style: { color: '#0ea5e9', weight: 3, fillOpacity: 0.1 } }).addTo(map.value)
...
const fetchShops = async () => {
  try {
    ...
    shops.value = data || []
    plotShops()
  } catch (e) { console.error(e); errorMsg.value = 'Failed to fetch shops.' }
...
const getFullAddress = (shop: any) => {
  ...
  return parts.length ? parts.join(', ') : shop.physical_store ?? 'Address not available'
}
...
const plotShops = async () => {
  if (!map.value || !shops.value.length) return
  ...
  for (const shop of shops.value) {
    ...
    let distanceKm = getDistanceInKm(userLat, userLng, lat, lng)
    try { distanceKm = await computeRouteDistance([userLat, userLng], [lat, lng]) } catch {}

    (shop as any).distanceKm = distanceKm

    const marker = L.marker([lat, lng], { icon: registeredShopIcon, title: shop.business_name }).addTo(map.value!)
...
onMounted(async () => {
  ...
  try {
    ...
    void updateShops()
  } catch { console.warn('Quick geolocation failed') }
  map.value?.whenReady(async () => { await startWatching() })
})

onUnmounted(() => { stopWatching(); if (latitude.value && longitude.value) saveCachedLocation(Number(latitude.value), Number(longitude.value)); if (map.value) setTimeout(() => map.value?.remove(), 300) })
...
const routeToShop = (shop: any) => {
  ...
  if (routingControl) { map.value.removeControl(routingControl); routingControl = null }
  routingControl = L.Routing.control({
    ...
    lineOptions: { styles: [{ color: '#1E90FF', weight: 6, opacity: 0.9 }, { color: 'white', weight: 3, opacity: 0.8 }] },
    createMarker: (i, wp) => i === 0 ? L.marker(wp.latLng, { icon: userIcon }).bindPopup('You are here') : L.marker(wp.latLng, { icon: registeredShopIcon }).bindPopup(shop.business_name)
  }).addTo(map.value)
  routingControl.on('routesfound', () => { const line = (routingControl as any)?._line; if (line) line.bringToFront() })
}

const computeRouteDistance = async (start: [number, number], end: [number, number]): Promise<number> => {
  return new Promise((resolve, reject) => {
    ...
    control.on('routesfound', (e: any) => { const route = e.routes[0]; const distanceKm = route.summary.totalDistance / 1000; map.value?.removeControl(control); resolve(distanceKm) })
    control.on('routingerror', (err) => { map.value?.removeControl(control); console.warn('Routing error', err); reject(err) })
  })
}
...
const fetchNearbyUnregisteredShops = async (lat: number, lon: number) => {
  try {
    const res = await fetch(`https://api.geoapify.com/v2/places?categories=shop&filter=circle:${lon},${lat},5000&apiKey=${GEOAPIFY_API_KEY}`)
    const data = await res.json()
    return data.features || []
  } catch (err) { console.error('Failed to fetch unregistered shops:', err); return [] }
}
const plotUnregisteredShops = (places: any[]) => {
  for (const place of places) {
    const lat = place.geometry.coordinates[1]
    const lon = place.geometry.coordinates[0]
    const marker = L.marker([lat, lon], { icon: unregisteredShopIcon, title: place.properties.name || 'Unregistered Shop' }).addTo(map.value!)
    marker.bindPopup(`<div style="text-align:center;"><p><strong>${place.properties.name || 'Unregistered Shop'}</strong></p><p style="margin:2px 0; font-size:14px;">${place.properties.address_line1 || ''}</p></div>`)
    shopMarkers.push(marker)
  }
}
const updateShops = async () => {
  await fetchShops()
  if (latitude.value && longitude.value) { const unregistered = await fetchNearbyUnregisteredShops(latitude.value, longitude.value); plotUnregisteredShops(unregistered) }
}
</script>
